...
$db->query("select * from juegos where genero = '".$genero."'");
...
?>
    ...
    <?php
// Crear una conexión a la base de datos con las credenciales del usuario
$db = new mysqli("localhost", "root", "", "trabajofin");
// Comprobar si hubo un error al conectar a la base de datos, y si lo hubo, imprimir un mensaje de error y detener el script
if ($db->connect_errno) {
  die("Error al conectar a la base de datos: " . $db->connect_error);
}
// Obtener el valor de la variable POST "genero"
$genero = $_POST["genero"];
// Ejecutar una consulta para seleccionar todos los juegos en la tabla "juegos" que tengan el género especificado
$resultados = $db->query("SELECT * FROM juegos WHERE genero = '$genero'");
// Imprimir un encabezado con el género de juegos que se está buscando
echo "<br><h2 style='font-family: Comic Sans MS, sans-serif; font-size: 30px; color: white; text-align: center;'>Resultados de búsqueda para el género $genero:</h2><br><br><br>";
echo "<table>";
echo "<tr style='font-family: Comic Sans MS, sans-serif; font-size: 25px; color: white; text-align: center;'><th>Imagen</th><th>Nombre</th><th>Plataforma</th><th>Link</th></tr>";
// Iterar sobre cada resultado de la búsqueda, y agregar una fila a la tabla para cada uno
while ($juego = $resultados->fetch_assoc()) {
  $ruta_imagen = "imgs/imagen_" . $juego["id_juego"] . ".png";
  echo "<tr style='font-family: Comic Sans MS, sans-serif; font-size: 15px; font-weight: bold; color: white; text-align: center;'><td><img src='" . $ruta_imagen . "' width='100' height='100'></td><td>" . $juego["nombre"] . "</td><td>" . $juego["plataforma"] . "</td><td><a href='" . $juego["link"] . "'>" . $juego["link"] . "</a></td></tr>";
}
echo "</table>";
