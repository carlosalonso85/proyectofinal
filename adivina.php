...
<head>
	<title>ADIVINA</title>
    <link rel="shortcut icon" href="imgs/juegos-de-mesa.png">
...
<body>
    <div class="adivina">
    <h1 style="color:white;">Adivina el juego</h1>
    <br>

    <?php
    // Comprobamos si se ha enviado una respuesta por POST
    if (isset($_POST["respuesta"])) {
        // Comprobamos si la respuesta es correcta
        $respuesta = $_POST["respuesta"];
        if (comprobar_respuesta($con, $respuesta)) {
            echo "<h2 style='color: green'>¡Lo has adivinado!</h2>";
            echo "<p>Prueba de nuevo:</p>";
            // Si la respuesta es correcta, eliminamos la imagen actual de la sesión
            unset($_SESSION["imagen_actual"]);
        } else {
            echo "<h2 style='color: red'>Sigue intentándolo</h2>";
        }
    }
    // Si no hay una imagen actual en la sesión, obtenemos una imagen aleatoria
    if (!isset($_SESSION["imagen_actual"])) {
        $_SESSION["imagen_actual"] = obtener_imagen_aleatoria();
    }
    // Obtenemos la ruta de la imagen actual
    $imagen_path = "imgs/imagen_" . $_SESSION["imagen_actual"] . ".png";
    ?>
    <!-- Mostramos la imagen actual en la página -->
    <div class="imagen-container">
    <img src="<?php echo $imagen_path; ?>" alt="Imagen aleatoria">
</div>
    <form action="" method="post">
    <br><br>
        <label style="color:white;" for="respuesta">¿Qué juego es?</label>
